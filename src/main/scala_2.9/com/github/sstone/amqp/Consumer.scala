 ...
 */
class Consumer(listener: Option[ActorRef], autoack: Boolean = false, init: Seq[Request] = Seq.empty[Request], channelParams: Option[ChannelParameters] = None) extends ChannelOwner(init, channelParams) with Stash with RequiresMessageQueue[UnboundedDequeBasedMessageQueueSemantics] {
  ...
  override def connected(channel: Channel, forwarder: ActorRef) : Receive = LoggingReceive({
    ...
    case request@AddQueue(queue) => {
      ...
      sender !  withChannel(channel, request)(c => {
        ...
        log.debug("using consumer %s" format consumerTag)
        consumerTag
      })
    }
